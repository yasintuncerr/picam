 ...
 * own DNGWriter on the C++ side. This C file is now just a transport layer.
 *
...
// Thread to handle a client. It's now extremely simple.
static void *client_thread_func(void *arg) {
        ...
        if (!session->server->still_src) {
            ...
            if (still_source_capture(session->server->still_src) < 0) {
                ...
                if (session->captured_data.mem && !session->captured_data.error) {
                    // Just send the complete DNG file. No conversion, no thinking.
...
// (http_server_thread, http_capture_new, http_capture_destroy are unchanged)
// ... PASTE THE REST OF THE SERVER CODE HERE ...
static void *http_server_thread(void *arg) {
